...
# Host address
HOST_IP=16.103.150.137
EXT_HOST_IP=16.103.150.137
...
keystone service-create --name heat-cfn --type cloudformation --description 'Orchestration CloudFormation service'
...
create_endpoint () {
  case $1 in
    ...
    ;;
    orchestration)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':8004/' --adminurl 'http://'"$HOST_IP"':8004/' --internalurl 'http://'"$HOST_IP"':8004/'
    ;;
    cloudformation)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':8000/' --adminurl 'http://'"$HOST_IP"':8000/' --internalurl 'http://'"$HOST_IP"':8000/'
    ;;
  esac
}

for i in compute volume image object-store orchestration cloudformation identity ec2 network; do
